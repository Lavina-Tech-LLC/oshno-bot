	...
	return func(c tele.Context) error {
		...
		default:
			if q == "yes" {
				...
				return c.Send("Вся информация получена. Спасибо за ответы. Через некоторое время Вам позвонят наши сотрудники службы поддержки и решат Вашу программу. Мы рады работать с Вами!", models.MenuMarkupRu)
			}
		}
	}
}
